...
function readxlssheet()
    ...
    # df = XLSX.readxlsx("/home/yuanyiping/下载/task 9/master-3/inputdata/data2_for168h.xlsx") # for little case
    ...
    windcurve = df["winds_data"]
    Sheet9_list = string("A2", ":", "B", string(size(windcurve[:], 1)))
    windcurve = convert(Array{Float64,2}, windcurve[Sheet9_list])

    pvdata = df["pv_data"]
    Sheet10_list = string("A2", ":", "B", string(size(pvdata[:], 1)))
    pvdata = convert(Array{Float64,2}, pvdata[Sheet10_list])

    return unitsfreqparam, windsfreqparam, strogesystemdata, gendata, gencost, linedata, loadcurve, loaddata, windcurve, pvdata
...
function forminputdata(DataGen, DataBranch, DataLoad, LoadCurve, GenCost, UnitsFreqParam, StrogeData)
    ...
    tem_Gens_TU = DataGen[:, 9]
    tem_Gens_TD = DataGen[:, 10]
    Gens_TU, Gens_TD = zeros(NG, 1), zeros(NG, 1)
    for i in 1:NG
        Gens_TU[i, 1] = round(tem_Gens_TU[i, 1])
        Gens_TD[i, 1] = round(tem_Gens_TD[i, 1])
    end
    Gens_TU = Gens_TU[:, 1]
    Gens_TD = Gens_TD[:, 1]
    Gens_x0 = DataGen[:, 11]
    ...
    # NOTES rewritten load curves
    Loads_SumLoad = LoadCurve[:, 2] / 100
    Loads_PerLoad = zeros(ND, NT)
    for i in 1:NT
        Loads_PerLoad[:, i] = Loads_SumLoad[i, 1] .* Loads_Percent[:, 1] * 1.0
    end
    ...
    # re-normazied data
    config_param = config(1, 1, 1, 1, 1, 3, 0.0025, 0.0025, 1, 1, 1, 1e5, 1e5, 50, 0.01)
